...
document.getElementById('download-itinerary').addEventListener('click', function() {
    ...
    let itineraryContent = itineraryElement.innerText;
...
function generateItineraryAhmedabad(days) {
    ...
    const hotels = [
        { name: "Hotel Ramada", price: 4500 },
        { name: "Hotel Crown Plaza", price: 5000 },
...
function generateItineraryOutput(days, activities, hotels, city) {
    ...
    activities.slice(0, days * 2).forEach(activity => {
        output += `<p>Day ${activity.day}: Visit ${activity.site} at ${activity.time} (Cost: ${activity.cost}Rs)</p>`;
    });

    output += `<h3>Recommended Hotels (3000Rs - 5000Rs)</h3><ul>`;
    hotels.forEach(hotel => {
        output += `<li>${hotel.name} - ${hotel.price}Rs</li>`;
    });
    output += `</ul>`;
